...
Decoded results must explicitly include both `Lat` and `Lon`, using finite
coordinates within latitude `[-90, 90]` and longitude `[-180, 180]`, so omitted
or malformed upstream locations are not accepted as valid sensor data.
