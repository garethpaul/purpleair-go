...
ATTACKER_ROOT=/tmp/purpleair-go-attacker-root
TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/PurpleAir Go's [gate] XXXX")
...
for target in build check docs fmt lint race root-test test vet verify; do
...
printf '%s\n' "Makefile root tests passed: 30 target/override cases and 2 MAKEFILE_LIST rejection cases"
